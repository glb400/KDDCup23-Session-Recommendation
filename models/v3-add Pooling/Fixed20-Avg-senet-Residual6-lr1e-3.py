...
run = wandb.init(
    ...
    config={
        ...
        "epochs": 10,
        "batch_size": 256,
        "structure": "avg-fixed emb20-att-senet-residual 6-fc",
    })
...
class Model(nn.Module):
    def __init__(self, input_dim, output_dim, vocab_size):
        ...
        # use residual blocks
        self.residual_block1 = ResidualBlock(2 * input_dim)
        self.residual_block2 = ResidualBlock(2 * input_dim)
        self.residual_block3 = ResidualBlock(2 * input_dim)
        self.residual_block4 = ResidualBlock(2 * input_dim)
        self.residual_block5 = ResidualBlock(2 * input_dim)
        self.residual_block6 = ResidualBlock(2 * input_dim)
...
for epoch in range(10):
    ...
    # save the model
    torch.save(model.state_dict(), 'avg_20embs_fixed_model_v1_6blks.pth')

# # load the model
# model.load_state_dict(torch.load('avg_20embs_fixed_model_v1_6blks.pth'))
