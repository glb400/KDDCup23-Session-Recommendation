...
run = wandb.init(
    ...
    config={
        ...
        "epochs": 10,
        "batch_size": 256,
        "structure": "widen-avg-fixed100-att-senet-residual 6-fc",
    })
...
class SENet(nn.Module):
    ...
    def forward(self, input):
        ...
        prevs_id_num = input[:, :100]
        prevs_locale = input[:, 100:106]
        prevs_price = input[:, 106:127]
        next_id_num = input[:, 127:227]
        next_locale = input[:, 227:233]
        next_price = input[:, 233:254]
...
# create the model
# input_dim = train_data.shape[2] + (100 - 1)
# because id is converted to id_embedding, so the input_dim should be 127
model = Model( input_dim=127, output_dim=2, vocab_size=vocab_size ) 
model.to(device)
...
for epoch in range(10):
    ...
    # save the model
    torch.save(model.state_dict(), 'widen_avg_fixed100_model_v1_6blks.pth')


# # load the model
# model.load_state_dict(torch.load('widen_avg_fixed100_model_v1_6blks.pth'))
