...
# step1: get the new train_data
# step1.1: get the 100 next_item_predictions for each item in train_data using rule-baseline method
# step2: input these 100 strings into model & get the corresponding features of the predicted items in nontext_features.pt
# step3: concat these features and the feature of the ground truth(next item) in model
# step4: train the model using the above features and get their scores with the feature of previous items
# step5: calculate the RINCE loss and backpropagate
...
for epoch in range(10):
    ...
    train(model, train_data, train_mask, optimizer, criterion, df_validate)
    
    # save the model
    torch.save(model.state_dict(), 'RINCE_decayed_09_bce_nofixed_senet.pth')
    # save the whole model
    torch.save(model, 'RINCE_decayed_09_bce_nofixed_senet_model.pth')
